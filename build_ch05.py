...
EXPLICIT_EQ = {
    ...
    "5.165": (181, 504, 528),
    "5.186": (197, 57, 81), "5.187": (197, 92, 153), "5.188": (197, 472, 501),
    "5.199": (206, 73, 118), "5.200": (206, 137, 188), "5.201": (206, 214, 230),
    "5.202": (206, 230, 247), "5.203": (206, 267, 318), "5.204": (206, 493, 514),
    "5.205": (206, 552, 588),
}
...
FIGPAGE = {"5.1": 127, "5.2": 129, "5.3": 132, "5.4": 133, "5.5": 134, "5.6": 147,
           ...
           "5.23": 188, "5.24": 189, "5.25": 191, "5.26": 193, "5.27": 198, "5.28": 200,
           "5.29": 207}
MANUAL  = {"5.2": (129, 50, 72, 388, 583), "5.3": (132, 75, 172, 365, 335),
           ...
           "5.24": (189, 105, 408, 345, 474), "5.27": (198, 65, 338, 388, 535),
           "5.28": (200, 105, 248, 345, 553), "5.29": (207, 65, 152, 388, 425)}
TOPCUT  = {}
...
FIGCAP = {
 ...
 "5.23": ("模糊辨识流程", "Fuzzy identification procedure", 10),
 "5.24": ("模糊模型参数概览", "Overview of fuzzy model parameters", 11),
 "5.25": ("结构搜索流程图", "Flow chart of structure search", 9.5),
 "5.26": ("不与坐标轴对齐的数据及划分示例", "Example for data and partitioning, which is not aligned with the coordinate axis", 9),
 "5.27": ("串-并联与并联模型评价的概念与解释", "Concept and interpretation of serial-parallel and parallel model evaluation", 13),
 "5.28": ("人工神经网络辨识流程", "Artificial neural network identification procedure", 10),
 "5.29": ("自相关 Ree(l) 检验与互相关 Ruε(l) 检验示例（N=500，95% 置信带：±0.0877）", "Examples of auto-correlation and cross-correlation tests (N=500, 95% confidence band ±0.0877)", 13),
}
...
b.para("一旦辨识出 E′A、E′B、cvi、mt 和 σ，便可再次利用 4.4.4.3 节引入的关系更精确地确定液压"
       "伺服系统的固有频率和阻尼比。然而须注意，位置和压力信号的有限差分会在所得信号中引起"
       "（相位）延迟，必须加以校正，例如引入保持或时延环节。")
b.h3("5.7.4　一般模型的辨识", "Identification of General Models")
b.para("尽管 HSS 的行为高度非线性，文献中对非线性 HSS 模型辨识的关注却少得多；见 Yin"
       "（1994），他提出使用最大似然法。")

b.h2("5.8　模糊辨识", "Fuzzy Identification")
b.para("模糊模型的辨识由若干阶段组成。基本辨识过程与经典辨识相似，不过由于特定的模型结构"
       "需要一些扩充。图 5.23 给出图 5.2 的精简版本，它将指导本节的辨识工作流。下面只讨论"
       "模糊特有的辨识任务，并假设辨识过程的前两步（建模目标设定、预辨识）已完成。")
fig("5.23")
b.para("参数辨识步骤以原型的不同（随机）初值多次执行：用于放置原型的算法并不提供全局最优"
       "结果。用经典 LS 算法估计其余线性参数（估计基于串-并联模型评价）。非线性参数（如与隶属"
       "函数相关的参数）由用户提供并保持固定。通常在参数辨识步骤后就已得到充分参数化的模型；"
       "所得模型用于初始化优化步骤。应用非线性优化步骤可优化非线性参数，更重要的是可针对并联"
       "模型评价优化模型——后者通常提供更稳健的参数化和改进的多步超前预测。模糊模型的验证用"
       "与其他辨识方法相同的方法进行（见 5.10 节）。")
b.h3("5.8.1　引言与模型参数概览", "Introduction and Model Parameter Overview")
b.para("辨识方法遵循模糊规则的特殊结构：其前提部分与结论部分的组成要求执行专门的任务。前提"
       "定义模型空间的模糊划分，从而定义局部模型的（模糊）有效范围和叠加。就辨识而言，首先须"
       "作一些设计决策并辨识结构参数：定义特征向量、选择划分的模糊度、选择距离评估，并选择"
       "网格型或多维划分（假设隶属函数类型已选为概率型）。划分本身在第二个参数估计步骤中计算。"
       "若无专家知识，划分数通常通过比较不同选择的模型质量来确定。这些任务对常规辨识并不"
       "典型，将采用模糊聚类算法和统计方法来完成。结论包含动态局部模型（通常为 ARX 型），它们"
       "定义主要的动态行为；就辨识而言，首先须确定其结构（即回归向量的组成），其次须估计"
       "参数。")
b.para("归纳起来，下列参数涉及模型结构（另见图 5.24 的概览）：")
b.para("1.　定义模糊划分结构的参数：轴平行（“网格型”）或自由形式（“多维”）划分的选择；规则数 c"
       "（即划分/局部模型的数目）；特征向量 z 的组成；划分的模糊度 v（隶属函数的形状）；距离"
       "范数类型 ‖·‖v（隶属函数的形状）。", indent=False)
b.para("2.　定义局部动态模型结构的参数：每个输入量的死区时间 τ；所考虑历史输入的数目 m（对每个"
       "输入量）；所考虑历史输出的数目 n。", indent=False)
fig("5.24")
b.h3("5.8.2　结构辨识", "Structure Identification")
b.label("5.8.2.1　划分数与特征的选择（Selection of the Number of Partitions and Features）")
b.para("一个核心问题是划分数 c 的选择，它等价于所选模糊模型类型的隶属函数和模糊规则的数目。"
       "在模式分类中，已发展出若干用正确划分数对静态数据划分的方法，如：(i) 选 c 使模糊聚类"
       "所用目标函数（Bezdek, 1981）或其修改版本（Sugeno and Yasukawa, 1993）最小；(ii) 选 c "
       "使划分熵最小（Bezdek, 1981）；(iii) 使用分离指标（Dunn, 1974）；(iv) 按特征向量、特征值"
       "和协方差合并相似簇（Krishnapuram and Freg, 1992）。")
b.para("然而，动态系统的数据通常不具备成功应用这些方法所需的良好分离结构。实践中，划分数的"
       "确定是组合结构搜索的一部分，该搜索包括划分、特征和回归向量选择——无法相互独立地解决"
       "这些任务并获得最优结果。图 5.25 给出一个基本算法，其基本思想是从粗划分及相关模型开始，"
       "增大粒度直到预测变差。这一思想对网格型和多维划分都通用（如 Sugeno and Kang, 1988；"
       "Sugeno and Tanaka, 1991；Kroll and Agte, 1997；Nelles 等, 1998）。主要区别是：网格型划分"
       "在每个迭代步要求更复杂的组合搜索，而多维划分可通过更简单的连续学习/优化进行；另一"
       "方面，网格型划分不那么抽象，因而更易解释。")
fig("5.25")
b.para("为简化问题，所有局部模型具有相同结构，因而都使用相同的回归向量 φ(k)（结论）。关于结论"
       "的结构辨识任务是定义所考虑历史输出和输入的数目 n、m，以及可能的死区时间 τ；高级版本"
       "甚至确定单个显著项。对带 g 个输入的 MISO 模型，须为每个输入确定 m 和 τ。SISO 模型最"
       "显著结论项的确定可用 Kortmann（1989）的基于 OLS 的算法进行。基本物理考虑和相关方法"
       "也可用于确定死区时间。关于动态过程行为的简单启发式常能给出模型结构的良好初步估计："
       "非周期非线性行为用 m=1、n=1 处理，振荡行为用 m=2、n=2 处理。经验表明，关于 y 的时域"
       "比关于 u 的更显著：多数情况下 m=1 已足够，有时 m=2 提高模型质量，但 m 很少取更高值。"
       "特征向量（前提）z(k) 通常由专家选择，或使用完整回归向量。")
b.label("5.8.2.2　基于原型的度量聚类算法的距离范数与模糊度选择（Distance Norms and Fuzziness Choice）")
b.para("聚类算法用于辨识基于距离的划分。这里选用一类特定算法——基于原型的度量模糊聚类算法。"
       "大多数度量模糊聚类算法要求用户选择一个对所有簇通常一致有效的距离范数；相反，例如 "
       "Gustafson 与 Kessel（或模糊协方差）算法（Gustafson and Kessel, 1979）为每个簇计算一个"
       "形式矩阵 D。这些聚类算法最小化一个目标函数。在数据缩放不良时，幅值最大的特征可能"
       "支配聚类；理想情况下应由对非线性行为最显著的特征引导聚类。范数应根据数据中普遍存在"
       "的模式来选择。使用基于协方差矩阵的 Mahalanobis 范数获得了良好经验：由于等范数线随"
       "数据分布，它能产生椭球形旋转簇，如图 5.26 的例子所示。欧几里得范数作为更简单、能力"
       "较弱的替代被推荐。")
fig("5.26")
b.para("v 没有普遍最优的选择，因为那取决于数据中所含的结构和噪声。研究一些特定选择可使 v 的"
       "效应更明显（详细讨论参阅 Kroll, 1997）：v→1 时模糊划分变为清晰；v ∈ (1; 3) 时隶属函数"
       "连续可微（对应用基于梯度的优化方法很重要，且这一选择支持模糊聚类中的簇形成）；v 取"
       "接近 1（如 v=1.1）可能产生更好拟合辨识数据的模型，但这常因过拟合（在交叉验证中显现）；"
       "v 较大（如 v>2）会使规则显著重叠，通常降低可达到的预测质量；v→∞ 产生不希望的“点状”"
       "簇。实践中用 v ∈ [1.2; 1.6] 获得良好经验，良好的默认值是 1.5。")
b.h3("5.8.3　参数辨识（前提）", "Parameter Identification (Premise)")
b.para("已发展出多种基于原型的度量模糊聚类算法，它们通过无监督学习最小化目标函数。在模糊"
       "辨识语境中选择算法时要考虑的一些关键问题是：它使用局部变化还是全局一致的距离范数？"
       "它提供概率型还是可能性型隶属函数？其应用有多容易？其使用会产生多少模型参数？算法"
       "例子有：模糊 c 均值（FCM）、模糊 c 簇类（FCV）、模糊 c 椭圆型（FCE）、模糊协方差矩阵"
       "（FKM，也称 Gustafson and Kessel 算法）、Gath and Geva、可能性 FCM（PCM）和模糊 c 壳"
       "（FCS）。Gustafson 与 Kessel（1979）算法为每个簇计算形式矩阵 D，实践中这意味着模型"
       "参数数目显著增加，通常预测质量的相应提高并不足以证明其合理；简单的 FCM 算法是更好的"
       "折中。下面用 FCM 算法确定隶属函数的原型。原始算法（Bezdek, 1981）为分类目的而开发，"
       "其目标是确定 c 个簇，使给定 N 个数据点对 c 个簇的隶属值最小化“组内平方误差和（WGSS）"
       "准则”：")
eq("5.182")
b.para("FCM 计算“模糊划分矩阵”U：", indent=False)
eq("5.183")
b.para("显然，模糊聚类的目标是逐点隶属的定义；相反，模糊辨识的目标是产生参数化的隶属函数，"
       "因而不同。为完成这一不同目标，使用算法的一个修改版本（算法 5.3）（Kroll, 1997）：它聚焦"
       "于簇原型并把模糊划分矩阵用作辅助变量，而原始算法则反之。", indent=False)
b.label("算法 5.3　修改的 FCM 算法")
b.para("步骤 1（选择设计参数）。　指定簇数 c（2≤c≤N）、模糊度参数 v ∈ (1;∞)、距离范数 ‖·‖v"
       "（用于特征评估）、矩阵范数 ‖·‖M 和终止阈值 εT（用于算法终止条件）。", indent=False)
b.para("步骤 2（初始化）。　为 c 个原型指定初值：对每个特征确定辨识数据中出现的最小值和最大值，"
       "在这些区间内随机放置 c 个原型。置迭代步 l=0。", indent=False)
b.para("步骤 3（更新离散模糊划分）。　(a) 确定所有奇异点（到某原型距离为 0 的数据点）；(b) 计算"
       "所有数据点对所有簇的隶属 μi,k（按式给出，区分有无奇异点的情形）。", indent=False)
b.para("步骤 4（计算新原型）。　按隶属加权的样本平均更新各原型 vi。", indent=False)
b.para("步骤 5（检查终止条件）。　若相邻两次迭代原型变化的矩阵范数 < εT 则停止；否则 l=l+1，"
       "返回步骤 3。", indent=False)
b.para("可用扩展的 Hölder 范数（Lancaster and Tismenetsky, 1985）提供一个可用于终止准则的矩阵"
       "范数：")
eq("5.184")
b.para("其中下标 j 遍历集合 (vi) 的所有元素，这些集合含属于簇 i（即最接近簇心 vi）的所有数据样本"
       "的索引。典型设计参数选择为 p=2、CT=1。聚类结果依赖于初始化。FCM 的局部收敛性在 "
       "Hathaway 与 Bezdek（1988）中得到证明。执行 FCM 算法后即得到模糊划分，所有原型（以及"
       "所有隶属函数）都被完全确定。", indent=False)
b.h3("5.8.4　参数辨识（结论）", "Parameter Identification (Conclusion)")
b.para("剩下的任务是辨识局部模型的参数。要么用加权 LS 算法相互独立地辨识局部模型，要么把全局"
       "模型作为整体辨识。这里采用后者，因为它通常提供更好的预测质量；其缺点是这种改进的全局"
       "性能有时会因对全局误差的“盲目”数值优化而伴随局部模型行为变差。模糊模型的全局预测为：")
eq("5.185")
b.para("由于辨识问题对参数是线性的，参数向量 θ′ 的估计可用（非递推的）普通 LS 法计算（见 5.5.2 "
       "节）", indent=False)
eq("5.186")
b.para("其中", indent=False)
eq("5.187")
b.para("隶属值也可表示为加权因子并与回归向量分开保存，这时用加权 LS 法进行参数估计。或者可用 "
       "5.5.2 节讨论的递推变体，它对在线应用有用。注意，该估计方法计算的是串-并联模型评价（而"
       "非并联评价）的参数；后者只能用非线性优化方法实现，如下一节所讨论。", indent=False)
b.h3("5.8.5　优化", "Optimisation")
b.para("迄今模糊模型已被完全参数化。其底层目标是：(i) 为原型位置最小化模糊聚类的某个特定目标"
       "函数；(ii) 为单独进行的结论函数参数估计最小化串-并联模型评价的最小均方预测误差。虽然"
       "各单步本身产生最优结果，但所得非线性模糊模型参数化并非最优。本节讨论迄今用简单方法"
       "取得了什么、以及应用非线性优化方法可获得什么。一方面，优化方法实现起来复杂得多、可能"
       "花费多得多的计算时间，且已获得的模型质量通常已很出色；另一方面，用非线性优化方法可"
       "显著改善模型质量。因此，额外的参数优化并非默认的辨识步骤。")
b.label("5.8.5.1　优化的前景与陷阱（Perspectives and Pitfalls of Optimisation）")
b.para("迄今确定的模型参数化在 LS 意义下并非最优：划分对聚类算法的目标函数而言是最优的")
eq("5.188")
b.para("结论函数参数用“最小均方”计算，这意味着它们一般不是无偏的，其次它们是为串-并联模型"
       "评价确定的：", indent=False)
eq("5.189")
b.para("尽管如此，迄今得到的模型通常相当好，在许多情形下可能已足够。然而，通过最小化串-并联"
       "预测代价辨识的模型，对并联评价常表现出有限的预测质量（另见 Henson and Seborg, 1996）。"
       "并联模型评价时，预测输出被反馈为模型输入；串-并联评价时，由输入和测量的历史输出作"
       "单步预测。这一原理图示于图 5.27。", indent=False)
fig("5.27")
b.para("良好的并联评价性质对把模型用于仿真或长程预测（如基于模型的预测控制）很重要。非线性"
       "优化可用于为并联模型评价取得最优参数化：")
eq("5.190")
b.para("模型的所有连续参数（如距离范数和原型）都可调整。经验表明，把这一纯数值优化限制在原型"
       "位置和结论函数参数上通常是有利的。", indent=False)
b.label("5.8.5.2　实现（Implementation）")
b.para("从全局解和复杂度角度看，无导数优化方法（如遗传算法或进化策略）有吸引力；但从计算角度"
       "（包括可在可容忍时间内计算的内容）看，选择了基于梯度的方法，即便它们只得到局部最优。"
       "使用二阶导数的方法导致过高的解析工作量，因此逼近 Hessian 的拟二阶方法尤其受关注；其中 "
       "Levenberg 和 Levenberg-Marquardt 算法尤其有趣（见 5.6.3 节）。Levenberg-Marquardt 法被"
       "发现最高效（Nelles, 1995；Bernd, 1996；Bernd 等, 1997）。代价函数在 v ∈ (1; 3) 时连续可微，"
       "解析导数记录于 Kroll（1997）。")

b.h2("5.9　基于人工神经网络的辨识", "Identification with Artificial Neural Networks")
b.para("神经网络模型辨识的基本过程（见图 5.28）与经典辨识和模糊辨识相似（图 5.2 和 5.23）。下面"
       "只简要讨论一些神经网络特有的辨识任务。详细学习参阅 Sjöberg（1995）和 Nørgaard 等"
       "（2000）。注意，在神经网络文献中，估计称为训练或学习，验证称为测试或泛化。")
fig("5.28")
b.h3("5.9.1　人工神经网络架构的选择", "Selection of Artificial Neural Network Architectures")
b.para("下面假设网络类型已选定。在训练神经网络之前，模型结构（网络）选择阶段须对以下作出适当"
       "选择：(1) 回归向量的分量；(2) ANN 应有的隐层数以及每层所含单元数；(3) 激活函数（RBF "
       "网络则为径向基函数）。激活函数/基函数的选择和回归向量的选择的可能性已在 5.4.3 节给出。"
       "用多少层这一问题并不容易：原则上，有许多基函数时，一个隐层足以对大多数实际合理的系统"
       "建模（Cybenko, 1989；Barron, 1993）。使用多于一个隐层的 ANN 的主要缺点是实现、训练和"
       "统计分析变得复杂得多。把注意力限于单隐层全连接网络，确定足够隐单元数的常见（试错）"
       "工作过程是逐渐增加其数目并同时评估测试误差；当达到某个隐单元数、其上泛化增益不显著时，"
       "接受该 ANN。这类（自动）网络结构选择方法通常导致式 5.150 类型的优化问题，常用准则是 "
       "Akaike 最终预测误差（FPE）准则和 Akaike 信息准则（AIC）；见 5.10.3 节。")
b.h3("5.9.2　权的估计", "Estimation of Weights")
b.para("5.4.3 节所述神经网络的参数（即权）估计问题，可用与 5.5 节经典辨识（尤其结合 5.6 节优化"
       "算法）完全相同的方法提出和求解。")
b.label("5.9.2.1　多层感知器网络的参数估计（Parameter Estimation for MLP Networks）")
b.para("对 MLP 网络（它们是待辨识参数的非线性函数），建议的训练或学习（即参数估计）方法是基于"
       "优化算法的预测误差法（见 5.6 节）。Levenberg-Marquardt 算法因适合病态问题而被普遍推荐并"
       "广泛使用（另见 Nørgaard 等, 2000）。对两层 MLP 网络（即单隐层网络，见 5.4.3 节、图 5.15、"
       "式 5.96）")
eq("5.191")
b.para("输出灵敏度（以及梯度）的计算相对容易进行：", indent=False)
eq("5.192", "5.193")
b.para("利用这些解析表达式，可以适当方式构建梯度和 Hessian 矩阵。", indent=False)
b.label("5.9.2.2　反向传播算法（Back-propagation Algorithm）")
b.para("该算法在神经网络方面获得了显著关注和广泛使用（如 Rumelhart 等, 1986；Hertz 等, 1991）。"
       "它不过是计算预测器梯度（或输出灵敏度）的一种便利算法，即对式 5.96 求微分的链式法则，"
       "适用于带任意激活函数的一般多层前馈网络。BP 算法通常与梯度下降算法结合，也称为广义"
       "delta 规则。然而，MLP 网络中参数（权）以非线性方式出现，因此 BP 算法与梯度下降结合在"
       "确定最优权时很慢。所以必须采用（拟）二阶训练算法，如 Broyden-Fletcher-Goldfarb-Shanno "
       "算法和 Levenberg-Marquardt 算法。")
b.label("5.9.2.3　径向基函数网络的参数估计（Parameter Estimation for RBF Networks）")
b.para("使用 RBF 方法的优点是：一旦选定 RBF（即基函数 Ψ 连同其中心和宽度），剩下的就只是用 LS "
       "算法确定系数 λj,l。因此 RBF 网络近来作为 MLP 网络的替代日益流行。通过适当选择中心，简单"
       "的 RBF 网络模型可能提供与 MLP 网络相似的逼近能力。辨识 RBF 网络时最困难的任务是基函数"
       "中心的选择（Chen 等, 1990a,b, 1992）：它们应适当采样网络输入域并反映数据分布。中心和"
       "高斯函数标准差可随机选取或以合理方式选取，例如借助标准聚类或栅格方法（通常采用最近邻"
       "法）。更合适的是使用聚类算法，如均值跟踪算法（Warwick 等, 1995a,b）。注意，在 RBF 结构"
       "中使用模糊聚类算法导出与 5.4.2 节所述非常相似的模型，即 Takagi-Sugeno 型模糊模型，因此"
       "可应用与 5.8 节相同的参数估计技术。")
b.h3("5.9.3　网络架构的优化（生长/剪枝）", "Optimisation of Network Architecture (Growing/Pruning)")
b.para("迄今假设整体网络架构（带特定回归向量结构、固定层数/单元数）是先验确定的，训练算法只用"
       "来估计权。然而，由于维数灾难问题，这并不总是可行，尤其当输入空间高维时。辨识通常以"
       "参数过多的模型告终，从而导致估计权的高方差，这又直接引向 5.5.5 节已讨论的偏差/方差"
       "困境。除正则化技术外，搜索最优模型结构的方法已被提及为处理偏差/方差问题的另一途径。"
       "ANN 文献中已提出若干迭代模型精化过程（综述见 Murray-Smith, 1994；Johansen and Foss, "
       "1995）。一种常见方法是把相对简单的初始模型生长以包含更多建模能力；另一种是训练然后"
       "剪枝一个相对复杂的模型以产生更简约的模型。剪枝从足以描述系统的全连接网络架构开始，"
       "逐一移除连接直到得到最优架构；生长则反向进行。应用剪枝算法前，强烈建议先用一个略小于"
       "最优值（约 2–100 倍）的正则化参数进行粗略显式正则化（Nørgaard 等, 2000）。两个突出的"
       "剪枝算法是最优脑外科（Optimal Brain Surgeon）和最优脑损伤（Optimal Brain Damage）。"
       "最后注意，参数估计和结构优化算法都不保证找到全局最优；除全局最小外，可能存在许多接近"
       "最优的（良好）局部最小（Johansen and Foss, 1995）。")

b.h2("5.10　模型验证与模型结构比较", "Model Validation and Comparison of Model Structures")
b.para("“仅仅提出所考虑过程的一个模型是不够的——我们还必须有一个衡量其可靠性的尺度。模型"
       "验证是检验模型、评估其质量并可能否定其用于所讨论目的的过程。某种意义上，这可视为辨识"
       "的本质过程——估计阶段实际上只是提供可能通过验证这一针眼的候选模型的手段”（Ljung, "
       "1993）。事实上，准确拟合某个辨识数据集很少成为问题；但试图用测试数据集验证模型时，"
       "通常给出不那么令人满意的结果。模型验证关乎如下问题（Söderström and Stoica, 1989）：")
b.bullet("给定模型足够灵活吗？")
b.bullet("给定模型过于复杂吗？")
b.bullet("两个或多个候选中应选择哪种模型结构？")
b.bullet("考虑到指定目标，应接受还是拒绝该模型？")
b.para("在系统辨识领域，提供了大量（非线性）模型验证技术，如对残差的统计检验（相关性检验）、"
       "把损失函数作为模型阶数的函数来评估，以及对所辨识模型的仿真。下面描述其中一些检验和"
       "判据。这里要强调，为确定合适的模型结构，一般推荐结合使用统计检验和对相关信号/预测"
       "曲线的目视检查。")
b.h3("5.10.1　预测、仿真与交叉验证", "Prediction, Simulation and Cross-validation")
b.para("一个自然的起点是检验模型在仿真中重现“新鲜”数据（即未用于估计模型的数据）的能力。"
       "这确实是常见做法，也许是获得对模型信心（或否定模型）的最有用、最务实的方式（Ljung, "
       "1993）。在同一图中比较辨识和验证（或测试）数据集的预测与实际测量的简单曲线，就能对"
       "偏差提供初步但良好的理解。重要的是不要过分依赖对一步超前预测（即图 5.6 和 5.13 所示的"
       "串-并联模型评价）ŷ(k|θ) 的目视检查：这些“朴素”预测一般会接近测量输出，尤其当采样"
       "频率相对于系统动态较高时。换言之，小的预测误差显然并不意味着模型好。相反，应检查 l "
       "步超前预测（l>1），或对模型作纯仿真（即图 5.6 和 5.13 所示的并联模型评价），对应于无穷"
       "预测时域。这种验证在与模型预测控制策略相关时具有实际意义，将在 6.7.2 节讨论。")
b.h3("5.10.2　残差检验", "Residual Tests")
b.para("用残差检验进行模型验证基于这样的假设：若残差与过去数据的所有（线性和非线性）组合之间"
       "无相关，则信息很可能已从辨识数据集中提取完毕。残差是预测误差（式 5.108 中的 ε），反映"
       "未建模部分和扰动。由于实际上不可能检查所有可能的相关，通常只选少数自相关和互相关"
       "函数。Billings 及其同事表明，采样相关函数理想情况下应满足如下条件（Billings and Voon, "
       "1986；Billings 等, 1992；Billings and Zhu, 1994）。线性相关函数检验：")
eq("5.194", "5.195")
b.para("非线性相关函数检验：", indent=False)
eq("5.196", "5.197", "5.198", "5.199", "5.200")
b.para("其中", indent=False)
eq("5.201", "5.202")
b.para("以及", indent=False)
eq("5.203")
b.para("然而实践中，相关系数对所有滞后绝不会恰好为零。因此，分析相关函数的常见方式是计算它们"
       "并把它们对时间滞后（通常在区间 l ∈ [−25, 25] 内）绘成图（见图 5.29），以指示估计的相关"
       "是否显著。该图再补以渐近分布的置信区间（如 95% 或 99%），以检查函数是否在此区间内，"
       "即是否 −1.96/√N < R < 1.96/√N 或 −2.58/√N < R < 2.58/√N。")
fig("5.29")
b.h3("5.10.3　模型结构检验判据", "Model Structure Test Criteria")
b.para("建模与辨识中的一个首要思想是使模型尽可能简单。因此，模型质量应用性能指标（标量量）"
       "评估或比较，如归一化残差平方和（NRSS）")
eq("5.204")
b.para("或 Sachs 多重相关系数（0 < Rs < 1）（Sachs, 1978）", indent=False)
eq("5.205")
b.para("给出 NRSS 最小值或 Rs 最大值的模型最佳，故被选中。使用这些及其他判据的问题在于，它们"
       "指出的最佳模型含过多——常常是所有可能的——模型分量。为避免该问题，引入了计入模型"
       "复杂度（即参数数目）的性能指标（或结构选择判据），如 Theil 调整多重相关系数（0 < R̄s² < 1）"
       "（Theil, 1971）")
eq("5.206")
b.para("以及 Amemiya 相关系数（0 < R²A < 1）（Amemiya, 1980）", indent=False)
eq("5.207")
b.para("R̄s² 或 R²A 较高的模型优于较小的模型。借助这些性能指标，同时具有小残差和少参数的模型"
       "为最优。其他突出的模型性能指标是所谓的信息准则，如：", indent=False)
b.para("• 最终预测误差准则（FPE；Akaike, 1970）", indent=False)
eq("5.208")
b.para("• Akaike 信息准则（AIC；Akaike, 1972；Sakamoto 等, 1986）", indent=False)
eq("5.209")
b.para("• Khinchin 重对数律准则（LILC；Hannan and Quinn, 1979；Hannan and Rissanen, 1982）", indent=False)
eq("5.210")
b.para("• Bayes 信息准则（BIC；Kashyap, 1977）", indent=False)
eq("5.211")
b.para("在所有这些准则中，模型结构所含参数越多，第一项就越小；然而每个参数都带有方差惩罚，"
       "它会（以相应因子）贡献于期望的均方误差拟合。", indent=False)
b.h2("5.11　实现与软件工具", "Implementation and Software Tools")
b.para("不言而喻，HSS 模型的辨识带来了多种模型结构和估计算法的可能。因此，有必要执行许多"
       "实验以充分了解各种现有可能并最终加以改进。这需要繁琐的工作，没有涵盖各种可能并支持"
       "修改的工具几乎不可能完成。为此，已开发出许多动态系统辨识工具箱，运行于 "
       "MATLAB/Simulink 环境。商业 MATLAB 工具箱（http://www.mathworks.com/products）有：")
b.bullet("Curve Fitting Toolbox：进行模型拟合与分析。")
b.bullet("Optimisation Toolbox：求解标准和大规模优化问题。")
b.bullet("Signal Processing Toolbox：信号处理、分析与算法开发。")
b.bullet("System Identification Toolbox：一般（黑箱）线性模型的辨识（已实现 OE、ROE、PEM）。")
b.bullet("Frequency Domain Identification Toolbox：由时间响应或频率响应测量辨识线性 SISO 系统。")
b.bullet("IDCON Classic / IDCON Nonlinear Toolbox：分别辨识连续时间线性模型和（状态空间）非线性模型。")
b.bullet("Fuzzy Logic Toolbox：模糊逻辑系统的设计与仿真。")
b.bullet("Neural Network Toolbox：神经网络的设计与仿真。")
b.bullet("MMLE3 Identification Toolbox：用最大似然法或输出误差法辨识连续时间状态空间模型参数。")
b.bullet("Tai-Ji ID：自动闭环辨识工具箱，从数据预处理到模型验证，专为基于模型的过程控制（如 MPC 和线性鲁棒控制）开发。")
b.para("免费的 MATLAB 工具箱有：")
b.bullet("CONTIDNS Toolkit：特殊标准形非线性系统的连续时间辨识（Jelali and Spielmann, 1996；Jelali, 1997）；联系本书第一作者。")
b.bullet("FREQID Toolbox：（多变量）线性模型的频域辨识。")
b.bullet("NNSYSID：基于神经网络的系统辨识工具箱（Nørgaard, 2000a），用不同优化算法辨识（MISO）神经网络模型（MLP）。")
b.bullet("FMID：模糊建模与辨识工具箱（Babuška, 1998），由数据构造 Takagi-Sugeno 模糊模型的 MATLAB 函数集。")
b.bullet("State Space Identification (SMI) Toolbox：由采样输入-输出序列辨识多变量线性时不变（LTI）和多变量 Wiener 系统。")
b.bullet("FIMO：模糊辨识与建模工具包，辨识增强版离散时间 Sugeno 型模糊模型（Bernd and Kroll, 1998）；联系本书第二作者。")
b.para("文献中报道的另一个 MATLAB 工具（Nelles and Isermann, 1996；Nelles, 1997 等）是 "
       "LOLIMOT（局部线性模型树），用于辨识 Sugeno 型模糊模型。")
b.h2("5.12　本章小结", "Section Summary")
b.para("本章把系统辨识过程表述为若干阶段（实验、数据准备、模型结构选择、参数估计、模型验证），"
       "特别关注最常见动态模型架构（线性模型、非线性多项式模型、Takagi-Sugeno 模糊模型、MLP "
       "和 RBF 网络）的介绍及其性质的讨论，并描述了可应用于每个辨识阶段的不同技术。因此，本章"
       "宜以简要概述 HSS 模型辨识的通用工作流程作结，并视模型目的给出每阶段合适方法的具体"
       "建议。")
b.para("实验。　实验活动为后续辨识阶段提供基础。特别是，实验所得数据的信息含量是非常关键的"
       "问题。实验应至少激励模型使用时可能受影响的过程所有状态。须明确区分线性模型（PRBS 已"
       "足够）和非线性模型（需用 PRMS 以覆盖尽可能多的幅值-频率组合，即维数灾难）；见 5.2.1 节。"
       "一个显而易见但很重要的步骤是目视检查数据，以检测如非线性效应（不同水平和不同方向阶跃"
       "响应的不同阻尼、不同响应时间常数等）。")
b.para("模型结构选择/搜索。　这一步涉及选择带相应参数数目的模型族。本阶段一个好的工程原则是"
       "先试简单的东西——简单首先意味着选择待估参数数目少的模型。通过从较简单（即线性）到"
       "较复杂（即非线性）模型搜索直到找到满意者，通常可取得偏差与方差之间的良好折中。若有"
       "先验知识（如来自物理洞见），模型结构选择也可得到简化，因为“不应试图估计已知的东西”"
       "（Ljung, 1993）。例如可把半物理回归量纳入模型（如 5.7 节所述），得到灰箱模型；只有当这"
       "给出不满意结果时才应考虑黑箱结构。具体用哪种模型结构（经典多项式、模糊或神经）取决于"
       "手头应用、先验知识量以及设计者或用户的偏好和技能。多项式模型（5.4.1 节）是线性模型的"
       "直接经典扩展；其主要缺点在于倾向于振荡的内插和外推行为，尤其使用高次多项式时。由于"
       "透明性和能以模糊规则或局部线性模型形式纳入不精确（专家）知识，模糊模型（尤其 "
       "Takagi-Sugeno 型；见 5.4.2 节）为黑箱建模提供了良好替代。人工神经网络（ANN）模型也能"
       "提供准确的过程输出预测，但与 Takagi-Sugeno 模型不同，它们对解释不透明；ANN 模型"
       "（尤其 MLP）不受维数灾难之苦，因而适合高维问题，代价是训练速度慢、收敛到坏局部最优的"
       "风险高。")
b.para("模型参数估计。　这一阶段处理运用某些数学算法为所选模型结构找到合适参数的任务。要使用"
       "的估计算法在很大程度上取决于所选模型结构（线性与非线性、参数与非参数、离散时间与连续"
       "时间、确定性与随机性）。然而，主要区别在于参数线性的模型（其参数可用经典 LS 求解器一步"
       "轻易估计）与具有更一般结构、因而需要应用更复杂优化算法的模型之间——其中 "
       "Marquardt-Levenberg 算法是最流行且成功应用的方法。为避免非线性优化，可先把 HSS 模型"
       "线性化（这对许多实际情形已足够），或采用参数线性的多项式模型。")
b.para("模型验证。　这一点负责认可模型表示底层系统的适当性，这最好在仿真中展示。尽管有许多"
       "验证方法可用（5.10 节），一个合理且广为接受的方法是采用交叉验证，即用未用于辨识的"
       "数据集在并联模型评价中验证模型。这相当合乎逻辑，因为我们基本上感兴趣的是覆盖过程特性"
       "的模型，而不只是重现某个特定时间序列。")

os.makedirs("parts", exist_ok=True)
b.save("parts/ch05.docx")
print("Saved parts/ch05.docx (COMPLETE: 5.1-5.12)")
